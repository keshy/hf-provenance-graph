...
class ModelCrawler:
    ...
    def crawl(self, starting=None, label='model', depth=1000, parent=None):
        if not starting:
            ...
            for model in models:
                ...
                # persist model
                datasets = []
                datasets.extend(model_d.get('training_datasets', ''))
            ...
            model = self.hf_api.model_info(starting)
            model_node = {
                'sha': model.sha,
                'tags': model.tags,
                'id': model.id,
...
if __name__ == "__main__":
    ...
    ac.crawl(starting=args.tag, label=args.label, depth=args.depth)
